...
{
    // NSInteger intorderPrice = [orderPrice integerValue];
    //订单标题，展示给用户
    NSString *order_name    = orderName;
    //订单金额,单位（分）
    //  NSString *order_price   = [NSString stringWithFormat:@"%.0f", intorderPrice * 100];//1分钱测试
    
    //设置回调函数
    NSString *notifyUrl = FZ_NOTIFY_URL;
    if ([notifyUrlType isEqualToString:@"ZC"]) {
        notifyUrl = ZC_NOTIFY_URL;
    }
    ...
    [packageParams setObject: appid             forKey:@"appid"];       //开放平台appid 必需
    [packageParams setObject: mchid             forKey:@"mch_id"];      //商户号 必需
    // [packageParams setObject: token             forKey:@"attach"];      //附加数据 非必需
    //[packageParams setObject: @"APP-001"        forKey:@"device_info"]; //支付设备号或门店号 非必需
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串 防重发 必需
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP 必需
    [packageParams setObject: order_name        forKey:@"body"];        //订单商品描述，展示给用户 必需
    [packageParams setObject: notifyUrl        forKey:@"notify_url"];  //支付结果异步通知 必需
    [packageParams setObject: orderno           forKey:@"out_trade_no"];//商户订单号 必需
    [packageParams setObject: @"196.168.1.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip 必需APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP
    [packageParams setObject: orderPrice       forKey:@"total_fee"];       //订单金额，单位为分必需
